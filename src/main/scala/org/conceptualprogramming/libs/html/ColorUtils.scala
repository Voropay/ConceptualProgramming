...
object ColorUtils {
 val extendedColorList = List(
  ...
  def computeMSE(color1: (String, Int, Int, Int), color2: (Int, Int, Int)): Int = {
    ((color1._2 - color2._1) * (color1._2 - color2._1) + (color1._3 - color2._3) * (color1._3 - color2._3) +
      (color1._4 - color2._3) * (color1._4 - color2._3))
  }

  def stringToRGBA(color: String): Option[(Int, Int, Int, Int)] = {
    val rgbaPattern = """rgba\((\d+),\s+(\d+),\s+(\d+),\s+(\d+)\)""".r
    val rgba = color match {
      case rgbaPattern(r, g, b, a) => Try((r.toInt, g.toInt, b.toInt, a.toInt)).toOption
      case _ => None
    }
    rgba
  }

  def extractColorName(color: String): String = {

 ...
 */
case class CPSubOperation(operand1: CPDependencyExpression, operand2: CPDependencyExpression) extends CPDependencyExpression {
  ...
  override def infer(result: CPValue, attributesValues: Map[CPAttributeName, CPValue]): Map[CPAttributeName, CPValue] = {
    ...
    if(value2.isEmpty && value1.isDefined) {
      val inferedValue = value1.get - result
      if(inferedValue.isDefined) {
  ...
  override def equals(other: Any) = {
    other match {
      case other: CPSubOperation => operand1 == other.operand1 && operand2 == other.operand2
      case _ => false
    }
  }
}
